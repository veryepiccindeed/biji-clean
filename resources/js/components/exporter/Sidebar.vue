<template>
    ...
    <div class="lg:hidden fixed top-0 inset-x-0 h-16 bg-[#1C1813] border-b border-[#2E241C] flex items-center justify-between px-5 z-40">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full border border-[#4A3728] flex items-center justify-center shrink-0" style="background: linear-gradient(135deg, #4B3832, #2C1F1A);">
                <span class="font-sans text-[14px] uppercase">{{ userName ? userName.charAt(0) : 'E' }}</span>
            </div>
            <p class="font-sans text-[16px] font-bold truncate max-w-[150px]">{{ userName || 'BIJI' }} <span class="font-normal text-[#A89880] text-[12px] ml-1">Exporter</span></p>
        </div>
    ...
    <aside :class="[
        ...
        <div class="flex items-center justify-between hidden lg:flex">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full border border-[#B8902A] flex items-center justify-center shrink-0" style="background: linear-gradient(135deg, #B8902A, #2D2210);">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-[#A89880]">Exporter</p>
                </div>
            </div>

            <button @click="$router.push({ name: 'exporter.pengaturan' })" class="relative w-8 h-8 rounded-lg flex items-center justify-center text-[#A89880] hover:text-[#F5EFE6] hover:bg-[#2A2118] transition focus:outline-none -mr-1" title="Pengaturan">
        ...
        <nav class="flex flex-col gap-2 text-[15px]">
            <router-link :to="{ name: 'exporter.dashboard' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'exporter.dashboard')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Dashboard</router-link>

            <details class="group" :open="isBatchOpen" @toggle="onDetailsToggle">
                <summary :class="summaryClass(currentRouteName.startsWith('exporter.batch-'))" class="px-3 py-2.5 lg:py-2 rounded-md transition cursor-pointer flex items-center justify-between">
                    <span>Batch</span>
                    <span class="text-[#5C4F42] group-open:rotate-180 transition">^</span>
                </summary>
                <div class="mt-2 ml-3 flex flex-col gap-2 text-[13px]">
                    <router-link :to="{ name: 'exporter.new-batch' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'exporter.new-batch')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Tambah Batch Baru</router-link>
                    <router-link :to="{ name: 'exporter.batch-tersedia' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'exporter.batch-tersedia')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Batch Tersedia</router-link>
                    <router-link :to="{ name: 'exporter.batch-saya' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'exporter.batch-saya')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Batch Saya</router-link>
                </div>
            </details>
...
// computed that prefers a manual user toggle, but will auto-open for batch routes
const isBatchOpen = computed({
    get() {
        if (uiStore.hasManualBatchState) return uiStore.isBatchOpen;
        return (
            uiStore.isBatchOpen ||
            routeName.value.startsWith('exporter.batch-') ||
            routeName.value === 'exporter.batch-detail' ||
            routeName.value === 'exporter.batch-detail2'
        );
    },
    set(val) {
        uiStore.setBatchOpen(val);
    }
});
...
function onDetailsToggle(e) {
    try {
        ...
        uiStore.setBatchOpen(open);
    } catch (err) {
        uiStore.setBatchOpen(!uiStore.isBatchOpen);
    }
}
...
const summaryClass = (isActive) =>
    isActive
        ? 'text-[#F5EFE6] bg-[#2A2118] font-semibold'
        : 'text-[#A89880] hover:text-[#F5EFE6] hover:bg-[#2A2118]';
