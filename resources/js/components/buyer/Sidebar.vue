<template>
    ...
    <div class="lg:hidden fixed top-0 inset-x-0 h-16 bg-[#1C1813] border-b border-[#2E241C] flex items-center justify-between px-5 z-40">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full border border-[#B8902A] flex items-center justify-center shrink-0" style="background: linear-gradient(135deg, #B8902A, #2D2210);">
                <span class="font-sans text-[14px] uppercase">{{ userName ? userName.charAt(0) : 'B' }}</span>
            </div>
            <p class="font-sans text-[16px] font-bold truncate max-w-[150px]">{{ userName || 'BIJI' }} <span class="font-normal text-[#A89880] text-[12px] ml-1">Buyer</span></p>
        </div>
    ...
    <aside :class="[
        ...
        <div class="flex items-center justify-between hidden lg:flex">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full border border-[#B8902A] flex items-center justify-center" style="background: linear-gradient(135deg, #B8902A, #2D2210);">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-[#A89880]">Buyer</p>
                </div>
            </div>

            <button @click="$router.push({ name: 'buyer.settings' })" class="relative w-8 h-8 rounded-lg flex items-center justify-center text-[#A89880] hover:text-[#F5EFE6] hover:bg-[#2A2118] transition focus:outline-none -mr-1" title="Pengaturan">
        ...
        <nav class="flex flex-col gap-2 text-[15px]">
            <router-link :to="{ name: 'buyer.dashboard' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'buyer.dashboard')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Dashboard</router-link>
            <router-link :to="{ name: 'buyer.catalog' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'buyer.catalog')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Katalog Kopi</router-link>
            <router-link :to="{ name: 'buyer.orders' }" @click="uiStore.closeMobileMenu()" :class="linkClass(currentRouteName === 'buyer.orders')" class="px-3 py-2.5 lg:py-2 rounded-md transition">Pesanan Saya</router-link>
